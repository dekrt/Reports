...
int main()
{
    // Open the device with read, write, and non-blocking permissions
    ...
    while (1)
    {
        ...
        // While the write operation returns 0 (indicating that no characters were written)
        while (write(fd, buffer, Num) == 0)
        {
            // Print the process ID and the number of characters requested to write
